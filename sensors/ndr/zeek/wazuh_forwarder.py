...
ZEEK_LOG_PATH = os.getenv(
    "ZEEK_LOG_PATH",
    "/var/log/zeek/current/conn.log.json",
)
...
SENSOR_EVENTS_INDEX = os.getenv("ZEEK_EVENTS_INDEX", "cybernest-zeek-events")
ACTION_RULE_GROUP = os.getenv("ZEEK_ACTION_RULE_GROUP", "zeek-response")
...
def read_zeek_events(log_path: str) -> List[Dict[str, Any]]:
    """Read Zeek JSON logs (one JSON per line) and return as list of dicts."""
...
def normalize_zeek_event(ev: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "sensor_type": "zeek",
        "sensor_id": ev.get("uid", "unknown"),
        "timestamp": int(time.time()),
        "raw": ev,
    }


def push_zeek_events_to_wazuh_indexer(
    indexer: WazuhIndexerAPI,
    log_path: str = ZEEK_LOG_PATH,
) -> List[Dict[str, Any]]:
    events = read_zeek_events(log_path)

    normalized_events: List[Dict[str, Any]] = []
    for ev in events:
        doc = normalize_zeek_event(ev)
...
def fetch_and_handle_actions(
    ...
    for action in actions:
        ...
        # Extend this to call Zeek scripts / control interfaces
        # to, for example, change capture filters or raise notices.
        print("[ZEEK][ACTION]", json.dumps(action, indent=2))


def main() -> None:
    indexer = WazuhIndexerAPI()

    pushed_events = push_zeek_events_to_wazuh_indexer(
        indexer=indexer,
        log_path=ZEEK_LOG_PATH,
    )
    print(f"Pushed {len(pushed_events)} Zeek events to index '{SENSOR_EVENTS_INDEX}'")

    sensor_id = os.getenv("ZEEK_SENSOR_ID", "zeek-main")
    since_ms = int(time.time() * 1000) - int(
        os.getenv("ZEEK_ACTION_LOOKBACK_MS", "300000")
    )
